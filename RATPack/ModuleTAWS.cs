...
namespace RATPack
{
	...
	public class ModuleTAWS: PartModule
	{
		...
		private bool IsGearDown()
		{
			...
			List<ModuleWheelDeployment> legs = vessel.FindPartModulesImplementing<ModuleWheelDeployment> ();
			foreach (ModuleWheelDeployment leg in legs) {
                if (leg.position == leg.deployedPosition)
                    return true;
			}
			return false;
		}
